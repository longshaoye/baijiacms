...
if ($sign == $array_data['sign']) {
    ...
    if ($array_data["return_code"] == "FAIL") {
        ...
        $paylog_weixin = array(
            'createtime' => time(),
            'mchId' => $_CMS['weixin_pay_mchId'],
            'timeend' => $time_end,
            'total_fee' => $total_fee,
            'openid' => $openid,
            'transaction_id' => $transaction_id,
            'out_trade_no' => $out_trade_no_str,
            'orderid' => $orderid,
            'ordersn' => $ordersn,
            'presult' => 'error',
            'order_table' => '',
        ...
        if (empty($index)) {
            ...
            $order = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE id = :id and ordersn=:ordersn", array(
                ':id' => $orderid,
                ':ordersn' => $ordersn
            ));
            if (! empty($order['id'])) {
                if ($order['status'] == 0) {
                    ...
                    mailnotice($orderid);
                    message('支付成功！', WEBSITE_ROOT . mobile_url('myorder', array(
                        'status' => 1
                    )), 'success');
                } else {
            ...
            $order = mysqld_select("SELECT * FROM " . table('gold_order') . " WHERE id = :id and ordersn=:ordersn", array(
                ':id' => $orderid,
                ':ordersn' => $ordersn
            ));
            if (! empty($order['id'])) {
                if ($order['status'] == 0) {
                    ...
                    mysqld_update('gold_order', array(
                        'status' => 1,
                        'paytime' => time()
                    ), array(
                        'id' => $order['id']

...
if (is_file(WEB_ROOT . '/config/install.link')) {
    ...
    header("location:" . web_url("index", array(
        "name" => "public"
    )));
    exit();
}
if ($op == 'setp2') {
    ...
    if (version_compare(PHP_VERSION, '5.3.0', 'ge')) {} else {
        $allcheck = false;
    }
...
EOF;
        ...
        header("Location:" . web_url("install", array(
            "name" => "public",
            "op" => "setp3",
            "doact" => "installsql",
            "adminname" => urlencode(base64_encode($_GP['adminname'])),
            "adminpwd" => urlencode(base64_encode(md5($_GP['adminpwd'])))
        )));
    }
    if ($_GP['doact'] == "installsql") {
        ...
        $data = array(
            'username' => base64_decode(urldecode($_GP['adminname'])),
            'is_admin' => 1,
            'password' => base64_decode(urldecode($_GP['adminpwd'])),
            'createtime' => time()
        );
        ...
        message("安装成功", web_url("index", array(
            "name" => "public"
        )), "success");
    }
}

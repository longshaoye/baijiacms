...
if (empty($settings['shop_category_style'])) {
    ...
    if (! empty($ppcateid)) {
        $categorypitem = mysqld_select("SELECT * FROM " . table('shop_category') . " WHERE deleted=0 and enabled=1 and id=:id ORDER BY parentid ASC, displayorder DESC", array(
            ":id" => intval($ppcateid)
        ));
        
        $categoryson = mysqld_selectall("SELECT * FROM " . table('shop_category') . " WHERE deleted=0 and enabled=1 and parentid=:parentid ORDER BY parentid ASC, displayorder DESC", array(
            ":parentid" => intval($ppcateid)
        ));
    }

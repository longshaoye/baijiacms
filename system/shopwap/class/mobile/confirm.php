...
$returnurl = ""; // 当前连接
$issendfree = 0;
$defaultAddress = mysqld_select("SELECT * FROM " . table('shop_address') . " WHERE isdefault = 1 and openid = :openid and deleted=0 limit 1", array(
    ':openid' => $openid
));
...
if (! empty($id)) {
    ...
    if (! empty($optionid)) {
        $option = mysqld_select("select title,marketprice,weight,stock from " . table("shop_goods_option") . " where id=:id", array(
            ":id" => $optionid
        ));
        if ($option) {
    ...
    // ========促销活动===============
    $promotion = mysqld_selectall("select * from " . table('shop_pormotions') . " where starttime<=:starttime and endtime>=:endtime", array(
        ':starttime' => TIMESTAMP,
        ':endtime' => TIMESTAMP
    ));
    ...
    $direct = true;
    $returnurl = mobile_url("confirm", array(
        "id" => $id,
        "optionid" => $optionid,
        "total" => $total
    ));
...
if (! $direct) {
    ...
    if (! empty($list)) {
        ...
        foreach ($list as &$g) {
            ...
            // 属性
            $option = mysqld_select("select * from " . table("shop_goods_option") . " where id=:id ", array(
                ":id" => $g['optionid']
            ));
            if ($option) {
...
if (checksubmit('submit')) {
    ...
    if ($direct && ! empty($item['isverify'])) {
        ...
        $address = mysqld_select("SELECT * FROM " . table('shop_address') . " WHERE id = :id", array(
            ':id' => intval($_GP['address'])
        ));
        if (empty($address)) {
            message('抱歉，请您填写收货地址！');
        }
    ...
    if (is_login_account()) {
        if (! empty($_GP['bonus'])) {
            ...
            $use_bonus = mysqld_select("select * from " . table('bonus_user') . " where deleted=0 and isuse=0 and  bonus_sn=:bonus_sn limit 1", array(
                ":bonus_sn" => $bonus_sn
            ));
            if (! empty($use_bonus['bonus_id'])) {
                
                $bonus_type = mysqld_select("select * from " . table('bonus_type') . " where deleted=0 and type_id=:type_id and    min_goods_amount<=:min_goods_amount and (send_type=0  or (send_type=1 ) or (send_type=2 and min_amount<:min_amount ) or send_type=3)  and use_start_date<=:use_start_date and use_end_date>=:use_end_date", array(
                    ":type_id" => $use_bonus['bonus_type_id'],
                    ":min_amount" => $goodsprice,
                    ":min_goods_amount" => $goodsprice,
                    ":use_start_date" => time(),
                    ":use_end_date" => time()
                ));
                if (! empty($bonus_type['type_id'])) {} else {
    ...
    $dispatchitem = mysqld_select("select sendtype,express from " . table('shop_dispatch') . " where id=:id limit 1", array(
        ":id" => $dispatchid
    ));
    $dispatchprice = 0;
    if ($issendfree != 1) {
    ...
    $randomorder = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE  ordersn=:ordersn limit 1", array(
        ':ordersn' => $ordersns
    ));
    if (! empty($randomorder['ordersn'])) {
        $ordersns = date('Ymd') . random(6, 1);
    }
    $payment = mysqld_select("select * from " . table("payment") . " where enabled=1 and code=:payment", array(
        ':payment' => $_GP['payment']
    ));
    if (empty($payment['id'])) {
        message("没有获取到付款方式");
        ...
        if (! empty($_GP['bonus'])) {
            ...
            if (! empty($use_bonus['bonus_id'])) {
                ...
                if (! empty($bonus_type['type_id'])) {
                    ...
                    mysqld_update('bonus_user', array(
                        'isuse' => 1,
                        'order_id' => $orderid,
                        'used_time' => time()
                    ), array(
                        'bonus_id' => $use_bonus['bonus_id']
                    ));
                    mysqld_update('shop_order', array(
                        'price' => $goodsprice + $dispatchprice - $bonusprice,
                        'hasbonus' => $hasbonus,
    ...
    foreach ($allgoods as $row) {
        ...
        $o = mysqld_select("select title from " . table('shop_goods_option') . " where id=:id limit 1", array(
            ":id" => $row['optionid']
        ));
        if (! empty($o)) {
    ...
    clearloginfrom();
    header("Location:" . mobile_url('pay', array(
        'orderid' => $orderid,
        'topay' => '1'
    )));
}
    ...
    foreach ($bonus_types as $bonus_type) {
        if (! empty($bonus_type['type_id'])) {
            if ($bonus_type['send_type'] == 0) {
                $bonus_users = mysqld_selectall("select * from " . table("bonus_user") . " where deleted=0 and  isuse=0 and bonus_type_id=:bonus_type_id and openid=:openid", array(
                ...
                foreach ($bonus_users as $bonus_user) {
                    ...
                    $bonus_list[] = array(
                        "bonus_sn" => $bonus_user['bonus_sn'],
                        "bonus_name" => $bonus_type['type_name']
                    );
                }
            }
            if ($bonus_type['send_type'] == 1) {
                foreach ($allgoods as $good) {
                    $bonus_good = mysqld_select("select * from " . table("bonus_good") . " where good_id=:good_id and bonus_type_id=:bonus_type_id limit 1", array(
                        ":good_id" => $good['id'],
                        ":bonus_type_id" => $bonus_type['type_id']
                    ));
                    if (! empty($bonus_good['id'])) {
                        $bonus_user = mysqld_select("select * from " . table("bonus_user") . " where deleted=0 and  isuse=0 and bonus_type_id=:bonus_type_id and openid=:openid limit 1", array(
                            ":bonus_type_id" => $bonus_type['type_id'],
                            ":openid" => $openid
                        ));

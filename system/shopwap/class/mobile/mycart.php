...
if ($op == 'add') {
    ...
    $goods = mysqld_select("SELECT id, type, total,marketprice,isverify FROM " . table('shop_goods') . " WHERE id = :id and deleted=0 and status=1", array(
        ':id' => $goodsid
    ));
    if (empty($goods)) {
            ...
            if ($op == 'update') {
                ...
                if (! empty($list)) {
                    foreach ($list as &$item) {
                        $goods = mysqld_select("SELECT  title, thumb, marketprice, total FROM " . table('shop_goods') . " WHERE id=:id limit 1", array(
                            ":id" => $item['goodsid']
                        ));
                        // 属性
                        $option = mysqld_select("select title,marketprice,stock from " . table("shop_goods_option") . " where id=:id limit 1", array(
                            ":id" => $item['optionid']
                        ));
                        if ($option) {

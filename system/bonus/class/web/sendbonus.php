...
if ($operation == 'post') {
    ...
    if ($bonus['send_type'] == 0) {
        ...
        if (checksubmit('send_user')) {
            foreach ($_GP['user_add'] as &$mobile) {
                $member = mysqld_select("SELECT * FROM " . table('member') . "where istemplate=0 and mobile=:mobile limit 1", array(
                    ":mobile" => $mobile
                ));
        ...
        if (checksubmit('search')) {
            $search_member_list = mysqld_selectall("SELECT * FROM " . table('member') . "where istemplate=0 and mobile like :mobile", array(
                ":mobile" => '%' . $_GP['send_user_tel'] . '%'
            ));
        }
        $user = $_GP['user'];
    ...
    if ($bonus['send_type'] == 1) {
        ...
        $bonus_good_list = mysqld_selectall("SELECT bonus_good.*,shop_goods.title FROM " . table('bonus_good') . " bonus_good left join " . table('shop_goods') . " shop_goods on shop_goods.id=bonus_good.good_id WHERE  bonus_good.bonus_type_id=:bonus_type_id", array(
            ":bonus_type_id" => intval($_GP['id'])
        ));
        
        $goodslist = mysqld_selectall("SELECT * FROM " . table('shop_goods') . " WHERE  deleted=0 $condition and status=1 and id not in (SELECT good_id FROM " . table('bonus_good') . " WHERE  bonus_type_id=:bonus_type_id)", array(
            ":bonus_type_id" => intval($_GP['id'])
        ));
        
        if (checksubmit('send_goods')) {
            
            mysqld_delete('bonus_good', array(
                'bonus_type_id' => intval($_GP['id'])
            ));
            foreach ($_GP['good_add'] as &$goodid) {
                $bonus_good = mysqld_select("SELECT * FROM " . table('bonus_good') . " WHERE  bonus_type_id=:bonus_type_id and good_id=:good_id", array(
                    ":bonus_type_id" => intval($_GP['id']),
                    ":good_id" => intval($goodid)
                ));
                
                $goods = mysqld_select("SELECT * FROM " . table('shop_goods') . " WHERE  deleted=0 and id=:goodid and status=1", array(
                    ":goodid" => $goodid
                ));
                if (empty($bonus_good['id']) && ! empty($goods['id'])) {
                    
                    mysqld_insert('bonus_good', array(
                        'bonus_type_id' => intval($_GP['id']),
                        'good_id' => $goodid
                    ));
                }
    ...
    if ($bonus['send_type'] == 3) {
        ...
        if (checksubmit('submit')) {
            ...
            message("发放成功！", create_url('site', array(
                'name' => 'bonus',
                'do' => 'bonus',
                'op' => 'display'
            )), "success");

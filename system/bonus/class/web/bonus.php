...
if ($operation == 'delete') {
    ...
    mysqld_update('bonus_type', array(
        'deleted' => 1
    ), array(
        'type_id' => intval($_GP['id'])
    ));
...
if ($operation == 'post') {
    ...
    if (checksubmit('submit')) {
        if (empty($_GP['id'])) {
            ...
            message("添加成功", create_url('site', array(
                'name' => 'bonus',
                'do' => 'bonus',
                'op' => 'display'
            )), "success");
            ...
            mysqld_update('bonus_type', $data, array(
                'type_id' => $_GP['id']
            ));
...
foreach ($bonus_list as $index => $bonus) {
    $bonus_list[$index]['sendcount'] = mysqld_selectcolumn("SELECT count(*) FROM " . table('bonus_user') . " where deleted=0 and bonus_type_id=:bonus_type_id", array(
        ":bonus_type_id" => $bonus['type_id']
    ));
    $bonus_list[$index]['usercount'] = mysqld_selectcolumn("SELECT count(*) FROM " . table('bonus_user') . " where deleted=0 and isuse=1 and bonus_type_id=:bonus_type_id", array(
        ":bonus_type_id" => $bonus['type_id']
    ));
}

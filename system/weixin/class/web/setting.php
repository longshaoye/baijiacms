...
$payment = mysqld_select("SELECT * FROM " . table('payment') . " WHERE code = :code", array(
    ':code' => 'weixin'
));
if (! empty($payment['configs'])) {
    $paymentconfig = unserialize($payment['configs']);
}
$thirdlogin = mysqld_select("SELECT * FROM " . table('thirdlogin') . " WHERE code = :code", array(
    ':code' => 'weixin'
));
...
if (checksubmit()) {
    ...
    if (empty($thirdlogin['id'])) {
        ...
        mysqld_update('thirdlogin', $data, array(
            'code' => 'weixin'
        ));
    }

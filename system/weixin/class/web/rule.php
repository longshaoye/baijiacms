...
if ($operation == 'detail') {
    ...
    if (! empty($_GP['id'])) {
        $rule = mysqld_select('SELECT * FROM ' . table('weixin_rule') . " WHERE  id = :id", array(
            ':id' => intval($_GP['id'])
        ));
    }
    ...
    if (checksubmit()) {
        ...
        if (empty($_GP['id'])) {
            $count = mysqld_selectcolumn('SELECT count(id) FROM ' . table('weixin_rule') . " WHERE  keywords = :keywords", array(
            ...
            mysqld_update('weixin_rule', $data, array(
                'id' => $_GP['id']
            ));
...
if ($operation == 'delete' && ! empty($_GP['id'])) {
    ...
    mysqld_delete('weixin_rule', array(
        'id' => $_GP['id']
    ));

...
class weixinAddons extends BjSystemModule
{
    ...
    public function verifyorder($openid, $ordersn)
    {
        ...
        if ($_CMS['addons_bj_hx']) {
            $item = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE ordersn = :ordersn	", array(
                ':ordersn' => $ordersn
            ));
            if (empty($item)) {
            ...
            $bj_hx_verify_saler = mysqld_select("SELECT * FROM " . table('bj_hx_verify_saler') . " WHERE openid = :openid", array(
                ':openid' => $openid
            ));
            ...
            if ($item['status'] > 0 && ! empty($item['isverify'])) {
                $shop_order_goods = mysqld_select("SELECT * FROM " . table('shop_order_goods') . " WHERE orderid = :orderid ", array(
                    ':orderid' => $item['id']
                ));
                $bj_hx_verify_goods = mysqld_select("SELECT * FROM " . table('bj_hx_verify_goods') . " WHERE goodsid = :goodsid  and verifyid=:verifyid", array(
                    ':goodsid' => $shop_order_goods['goodsid'],
                    ':verifyid' => $bj_hx_verify_saler['verifyid']
                ));
                if (! empty($bj_hx_verify_goods['goodsid'])) {
                    
                    return '<a href="' . WEBSITE_ROOT . create_url('mobile', array(
                        'name' => 'bj_hx',
                        'do' => 'verifycheck',
                        'ordersn' => $ordersn
                    )) . '">点击进入订单核销页面</a>';
    ...
    public function do_process()
    {
        ...
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
            ...
            if ($message['type'] == 'text' || $message['type'] == 'CLICK') {
                ...
                if ($_CMS['addons_bj_tbk']) {
                    ...
                    if (! empty($reply['addonsrule']) && ! empty($key) && $reply['addonsModule'] == 'bj_tbk') {
                        ...
                        $spread = mysqld_select("SELECT * FROM " . table('bj_tbk_qrcode') . " where weixinkey=:weixinkey limit 1", array(
                            ':weixinkey' => $key
                        ));
                        ...
                        if (! empty($spread['id'])) {
                            ...
                            if (! empty($weixin_wxfans['openid'])) {
                                ...
                                if (! empty($spread['allowqrcode'])) {
                                    $bj_tbk_member_relect = mysqld_select("SELECT fmr.* FROM " . table('bj_tbk_member_relect') . " as fmr where fmr.openid=:openid ", array(
            ...
            if ($message['type'] == 'subscribe') {
                ...
                $weixin_wxfans = mysqld_select('SELECT * FROM ' . table('weixin_wxfans') . "   WHERE  weixin_openid = :weixin_openid limit 1", array(

...
if ($operation == 'display') {
    ...
    if (checksubmit('submit')) {
        ...
        if (! empty($id)) {
            mysqld_update('shop_adv', $data, array(
                'id' => $id
            ));
        } else {
            mysqld_insert('shop_adv', $data);
        }
        message('更新幻灯片成功！', web_url('adv', array(
            'op' => 'display'
        )), 'success');
    }
    $adv = mysqld_select("select * from " . table('shop_adv') . " where id=:id  limit 1", array(
        ":id" => $id
    ));
    include page('adv');
    ...
    if (empty($adv)) {
        message('抱歉，幻灯片不存在或是已经被删除！', web_url('adv', array(
            'op' => 'display'
        )), 'error');
    }
    mysqld_delete('shop_adv', array(
        'id' => $id
    ));
    message('幻灯片删除成功！', web_url('adv', array(
        'op' => 'display'
    )), 'success');
} else {
    message('请求方式不存在');

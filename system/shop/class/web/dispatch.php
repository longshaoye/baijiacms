...
if ($operation == 'display') {
    $list = mysqld_selectall("SELECT * FROM " . table('shop_dispatch') . " where deleted=0 and express=:code ORDER BY displayorder desc", array(
        ':code' => $code
    ));
    foreach ($list as $index => $item) {
        $list[$index]['dispatch_areas'] = mysqld_selectall("SELECT * FROM " . table('shop_dispatch_area') . " WHERE dispatchid = :dispatchid", array(
            ':dispatchid' => $item['id']
        ));
    }
...
if ($operation == 'post') {
    ...
    $dispatch = mysqld_select("SELECT * FROM " . table('dispatch') . " where code=:code", array(
        ':code' => $code
    ));
    
    if (! empty($id)) {
        $item = mysqld_select("SELECT * FROM " . table('shop_dispatch') . " WHERE id = :id", array(
            ':id' => $id
        ));
        $dispatch_areas = mysqld_selectall("SELECT * FROM " . table('shop_dispatch_area') . " WHERE dispatchid = :dispatchid", array(
            ':dispatchid' => $id
        ));
    }
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($id)) {
            ...
            mysqld_update("shop_dispatch", $data, array(
                'id' => $id
            ));
        }
        
        mysqld_delete("shop_dispatch_area", array(
            "dispatchid" => $id
        ));
        foreach ($_GP['regions'] as $region) {
            $regions = explode("-", $region);
            ...
            mysqld_insert("shop_dispatch_area", array(
                'dispatchid' => $id,
                'country' => $country,
                'provance' => $provance,
                'city' => $city,
        ...
        message('配送方式操作成功！', create_url('site', array(
            'name' => 'shop',
            'do' => 'dispatch',
            'op' => 'display',
            'code' => $code
    ...
    $id = intval($_GP['id']);
    $row = mysqld_select("SELECT id FROM " . table('shop_dispatch') . " WHERE id = :id", array(
        ':id' => $id
    ));
    if (empty($row)) {
    ...
    // 修改成不直接删除
    mysqld_update("shop_dispatch", array(
        'deleted' => 1
    ), array(
        'id' => $id
    ));

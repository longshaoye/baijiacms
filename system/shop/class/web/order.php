...
if ($operation == 'display') {
    ...
    foreach ($list as $id => $item) {
        $list[$id]['isguest'] = mysqld_selectcolumn("SELECT istemplate from " . table('member') . " where  openid=:openid ", array(
            ':openid' => $item['openid']
        ));
        ;
    }
...
if ($operation == 'detail') {
    ...
    $order = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE id=:id", array(
        ":id" => $orderid
    ));
    if ($order['hasbonus']) {
    ...
    $goods = mysqld_selectall("SELECT g.id,o.total, g.title, g.status,g.thumb, g.goodssn,g.productsn,g.marketprice,o.total,g.type,o.optionname,o.optionid,o.price as orderprice FROM " . table('shop_order_goods') . " o left join " . table('shop_goods') . " g on o.goodsid=g.id " . " WHERE o.orderid='{$orderid}'");
    $order['goods'] = $goods;
    ...
    if (checksubmit('cancelreturn')) {
        $item = mysqld_select("SELECT * FROM " . table('shop_order') . " WHERE id = :id", array(
            ':id' => $orderid
        ));
        $ostatus = 3;
        if ($item['status'] == - 2) {
    ...
    $weixin_wxfans = mysqld_selectall("SELECT * FROM " . table('weixin_wxfans') . " WHERE openid = :openid", array(
        ':openid' => $order['openid']
    ));
    $alipay_alifans = mysqld_selectall("SELECT * FROM " . table('alipay_alifans') . " WHERE openid = :openid", array(
        ':openid' => $order['openid']
    ));

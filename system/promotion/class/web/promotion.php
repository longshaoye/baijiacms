...
if ($operation == 'delete') {
    ...
    mysqld_delete('shop_pormotions', array(
        'id' => intval($_GP['id'])
    ));
...
if ($operation == 'post') {
    $id = intval($_GP['id']);
    $pro = mysqld_select("select * from " . table('shop_pormotions') . "where id=:id  limit 1", array(
        ':id' => $id
    ));
    if (checksubmit('submit')) {
        ...
        if (! empty($id)) {
            mysqld_update("shop_pormotions", $data, array(
                'id' => $id
            ));
        } else {
            mysqld_insert("shop_pormotions", $data);
        }
        message('操作成功！', create_url('site', array(
            'name' => 'promotion',
            'do' => 'promotion',
            'op' => 'display'
        )), 'success');

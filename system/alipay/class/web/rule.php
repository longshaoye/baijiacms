...
if ($operation == 'detail') {
    ...
    if (! empty($_GP['id'])) {
        $rule = mysqld_select('SELECT * FROM ' . table('alipay_rule') . " WHERE  id = :id", array(
            ':id' => intval($_GP['id'])
        ));
    }
    ...
    if (checksubmit()) {
        ...
        if (empty($_GP['id'])) {
            $count = mysqld_selectcolumn('SELECT count(id) FROM ' . table('alipay_rule') . " WHERE  keywords = :keywords", array(
                ':keywords' => $_GP['keywords']
            ));
            if ($count > 0) {
            ...
            mysqld_update('alipay_rule', $data, array(
                'id' => $_GP['id']
            ));
...
if ($operation == 'delete' && ! empty($_GP['id'])) {
    ...
    mysqld_delete('alipay_rule', array(
        'id' => $_GP['id']
    ));

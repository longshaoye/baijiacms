...
if ($operation == 'deletegroup') {
    mysqld_delete('user_group', array(
        'id' => $_GP['id']
    ));
    mysqld_delete('user_group_rule', array(
        'gid' => $_GP['id']
    ));
...
if ($operation == 'rule') {
    ...
    if (checksubmit('submit')) {
        ...
        message('权限保存成功！', web_url('usergroup', array(
            'op' => 'listgroup'
        )), 'success');
    }
...
if ($operation == 'usergroup') {
    $usergroup = mysqld_select("select * from " . table('user_group') . " where id=:id", array(
        'id' => $_GP['id']
    ));
    ...
    if (checksubmit('submit')) {
        ...
        if (empty($_GP['id'])) {
            ...
            message('用户组添加成功！', web_url('usergroup', array(
                'op' => 'listgroup'
            )), 'success');
            exit();
        } else {
            mysqld_update('user_group', array(
                'groupName' => $_GP['groupName']
            ), array(
                'id' => $_GP['id']
            ));
            message('用户组修改成功！', web_url('usergroup', array(
                'op' => 'listgroup'
            )), 'success');
            exit();
        }

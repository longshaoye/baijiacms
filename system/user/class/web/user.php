...
if ($operation == 'edituser') {
    ...
    $account = mysqld_select('SELECT * FROM ' . table('user') . " WHERE  id=:id", array(
        ':id' => $_GP['id']
    ));
    ...
    if (checksubmit('submit')) {
        ...
        if ($username != $_GP['username']) {
            $t_account = mysqld_select('SELECT * FROM ' . table('user') . " WHERE  username=:username and id!=:id limit 1", array(
        ...
        if (! empty($account['id'])) {
            $user_group = mysqld_select("select * from " . table('user_group') . "where id=:id", array(
                ":id" => intval($_GP['groupid'])
            ));
            $data = array(
                'username' => $_GP['username'],
                'is_admin' => intval($_GP['is_admin']),
                'groupid' => intval($_GP['groupid']),
                'groupName' => $user_group['groupName'],
                'createtime' => time()
            );
            
            mysqld_update('user', $data, array(
                'id' => $account['id']
            ));
            message('密码修改成功！', create_url('site', array(
                'name' => 'user',
                'do' => 'user',
                'op' => 'listuser'
            )), 'succes');
...
if ($operation == 'adduser') {
    ...
    if (checksubmit('submit')) {
        ...
        if ($username != $_GP['username']) {
            $t_account = mysqld_select('SELECT * FROM ' . table('user') . " WHERE  username=:username  limit 1", array(
                ':username' => $_GP['username']
            ));
            if (! empty($t_account['id'])) {
                message("该用户名已存在，不能重复使用");
        ...
        $account = mysqld_select('SELECT * FROM ' . table('user') . " WHERE  username=:username", array(
            ':username' => $_GP['username']
        ));
...
if ($operation == 'rule') {
    $id = intval($_GP['id']);
    $modules_list = mysqld_selectall("select * from " . table('modules') . "where name!=:name", array(
        ":name" => "bj_tbk"
    ));
    ...
    if (checksubmit('submit')) {
        ...
        message('权限保存成功！', web_url('user', array(
            'op' => 'listuser'
        )), 'success');
    }
